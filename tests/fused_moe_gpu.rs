...
use flashinfer_rs::{DType, FusedMoeBackend, FusedMoeCudarcOptions, fused_moe_cudarc};
...
#[test]
fn gpu_smoke_launch_fused_moe_swizzled_input_sf_false() {
    if !should_run_gpu_tests() {
        eprintln!("skipping GPU test (set FLASHINFER_RS_RUN_GPU_TESTS=1 to enable)");
        return;
    }
    ...
    let options = FusedMoeCudarcOptions {
    ...
    .expect("launch fused moe with swizzled_input_sf=false");
