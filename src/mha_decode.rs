...
impl MhaBatchPagedDecodeParams {
    ...
    pub fn validate(&self) -> Result<(), FlashInferError> {
        ...
        if self.paged_kv_indptr_host.len != self.paged_kv_indptr.len {
            return Err(FlashInferError::invalid_argument(
                "paged_kv_indptr_host length must match paged_kv_indptr length",
            ));
        }
        ...
        if i64::from(host_batch) != batch_size {
            return Err(FlashInferError::invalid_argument(format!(
                "paged_kv_indptr_host batch size ({host_batch}) must equal q.dim0 ({batch_size})"
            )));
        }
...
pub fn mha_batch_decode_paged(params: &MhaBatchPagedDecodeParams) -> Result<(), FlashInferError> {
    params.validate()?;
    let runtime = FlashInferRuntime::global()?;
    // SAFETY: FFI preconditions are validated by `params.validate` and runtime initialization.
    unsafe { mha_batch_decode_paged_with_runtime(runtime, params) }
}

unsafe fn mha_batch_decode_paged_with_runtime(
    runtime: &FlashInferRuntime,
    params: &MhaBatchPagedDecodeParams,
) -> Result<(), FlashInferError> {
    let kernel_uri = params.kernel_uri();

    let batch_size = params.q.dim0;
...
#[cfg(feature = "cudarc")]
#[allow(clippy::too_many_arguments)]
pub fn mha_batch_decode_paged_cudarc<T, Q, K, V, O, PI, I, L, FW, IW>(
    stream: &cudarc::driver::CudaStream,
    q: &Q,
    paged_k_cache: &K,
    paged_v_cache: &V,
...
mod tests {
    ...
    #[test]
    fn batch_validate_rejects_paged_indices_length_mismatch() {
